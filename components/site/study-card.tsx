...
export function StudyCard({
  ...
  const statusClass = {
    ...
    completed: "bg-muted text-sub-foreground",
  }[status];
  ...
  return (
    <div className="border border-border rounded-lg p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-sm font-medium px-2 py-0.5 rounded-sm ${statusClass}`}
        >
          {statusLabel}
        </span>
      </div>
      <div className="space-y-1 text-sm text-sub-foreground">
        <p className="flex items-center gap-2">
          <Calendar className="w-4 h-4" strokeWidth={1.5} /> {date}
        </p>
        <p className="flex items-center gap-2">
          <Users className="w-4 h-4" strokeWidth={1.5} /> {enrolled}/{capacity}명 모집
        </p>
        <p className="flex items-center gap-2">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-sub-foreground text-right">{progress}% 모집</p>
      </div>
