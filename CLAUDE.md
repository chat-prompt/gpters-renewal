...
**사이트 핵심** — 홈(`/`), 탐색(`/explore/feed`), 커뮤니티(`/community/feed`), 게시글(`/posts/[slug]`), 글쓰기(`/write`), 검색(`/search`), 태그(`/tag/[slug]`), 메시지(`/messages`)
...
├── ui/    (19개)       # shadcn/ui 기반: button, card, select, avatar, table, input, checkbox, textarea 등
├── site/  (41개)       # 사이트: post-card, feed-post, search-input, sidebar-study-list 등
...
**핵심 공용 컴포넌트:**
- `site/post-card` — Medium 스타일 게시글 카드 (탐색/검색/태그 전체에서 공용)
